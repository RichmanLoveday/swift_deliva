        ...
        <div id="drivers_tabs" class="flex justify-between flex-row items-center">
            <div id="order-tab" class="flex flex-row justify-center space-x-10 items-center w-full">
                <div class="flex justify-between items-center active__nav tab" data-tab="tab 1">
                    <h3 data-userID="<?= $userID ?>" data-url="<?= ROOT ?>dispatch" data-companyID="<?= $companyID ?>"
                        class="px-3 font-sans text-[15px] cursor-pointer font-semibold leading-10 tab_head">
                        Current
                        Orders
                    </h3>
                    <span
                        class="flex items-center justify-center rounded-full h-5 w-5 text-xs px-2 py-1 text-white bg-yellowColor transition ease-linear duration-300">1</span>
                </div>
                <div class="flex justify-between items-center tab" data-tab="tab 2">
                    <h3 data-userID="<?= $userID ?>" data-url="<?= ROOT ?>dispatch/waiting_orders"
                ...
                <div class="flex justify-between items-center tab" data-tab="tab 3">
                    <h3 data-userID="<?= $userID ?>" data-url="<?= ROOT ?>dispatch/completed_orders"
                        data-companyID="<?= $companyID ?>"
                        class="px-3 font-sans text-[15px] cursor-pointer font-semibold leading-10 tab_head">
                        Completed
                        Orders
    ...
    <div class="-mt-5 px-2 w-full overflow-visible">
        <div id="driverOrdersList" class="flex flex-row flex-wrap justify-around mb-11 p-5 items-start w-full">
            ...
            <?php foreach ($orders as $order) : $inc++;
                    ...
                    if ($order->paymentMethod == 'Cash on delivery' && $order->orderStatus == 'done') {
            ...
            <div class="overflow-x-visible w-[30%] mt-5 rounded-md shadow-md bg-white px-4">
                <div class="flex justify-between px-5 pb-2 pt-5 items-center border-b border-gray-300/50 mb-3">
                    ...
                    <span class="float-right"><img src="<?= IMAGES ?>naira.png" class="w-3 h-3 inline-block -mt-1"
                            alt=""><?= $order->amount ?></span>
                </div>
                <!-- start poiint and end --->
                <div class="flex flex-col justify-start items-start text-left">
                    <span class="bg-primary-100 self-end px-3 py-1 rounded-full <?= $statusCol ?> text-xs font-"
                    ...
                    <button type="button" data-orderID="<?= $order->orderID ?>"
                        data-url="<?= ROOT ?>dispatch/view_order" data-userid="<?= $userID ?>"
                        ...
                        data-te-ripple-init data-te-ripple-color=""">View
             more <img src=" <?= IMAGES ?>yellow_arrow_down.png" alt="" class="inline-block ml-2"> </button>
                    <div class="w-full flex flex-row justify-around items-center">
                        ...
                        <img src="<?= IMAGES ?>change_status.png"
                            class="w-7 h-7 hover:scale-105 transition-transform ease-linear -mt-4 status_btn <?= $display ?>"
                            alt="">
                        <button type="button" <?= $driverCollectMoney ?> data-userid="<?= $userID ?>"
                            data-packageID="<?= $order->packageID ?>" data-orderID="<?= $order->orderID ?>"
                            data-url="<?= $url ?>" data-status="<?= $order->orderStatus ?>"
...
<section>
    ...
    <div data-te-modal-init
        class="fixed left-0 top-[30%] z-[1055] hidden h-full w-full overflow-y-auto overflow-x-hidden outline-none"
        ...
        <div data-te-modal-dialog-ref
            ...
            <div
                class="min-[576px]:shadow-[0_0.5rem_1rem_rgba(#000, 0.15)] pointer-events-auto relative flex w-full flex-col rounded-md border-none bg-white bg-clip-padding text-current shadow-lg outline-none bg-neutral-600">
                <div
                    class="flex flex-shrink-0 items-center justify-end rounded-t-md border-b-2 border-neutral-100 border-opacity-100 p-4 border-opacity-50">
                    <!--Close button-->
                    <button type="button"
                        class="box-content close_btn rounded-none border-none hover:no-underline hover:opacity-75 focus:opacity-100 focus:shadow-none focus:outline-none"
                ...
                <!--Modal footer-->
                <div
                    class="flex flex-shrink-0 flex-wrap items-center justify-end rounded-b-md border-t-2 border-neutral-100 border-opacity-100 p-4 border-opacity-50">
                    <button type="button"
                        class="inline-block rounded bg-blue-100 px-6 pb-2 pt-2.5 text-xs font-medium uppercase leading-normal text-primary-700 transition duration-150 ease-in-out hover:bg-primary-accent-100 focus:bg-primary-accent-100 focus:outline-none focus:ring-0 active:bg-primary-accent-200"
                        data-te-modal-dismiss data-te-ripple-init data-te-ripple-color=""">
             Close
                    </button>
                    <button id=" driverPay" type=" button" data-url="<?= ROOT ?>dispatch/update_status"
                        onclick="driver_accept_payment(this)"
                        class="ml-1 inline-block rounded bg-blue-700 px-6 pb-2 pt-2.5 text-xs font-medium uppercase leading-normal text-white shadow-[0_4px_9px_-4px_#3b71ca] transition duration-150 ease-in-out hover:bg-primary-600 hover:shadow-[0_8px_9px_-4px_rgba(59,113,202,0.3),0_4px_18px_0_rgba(59,113,202,0.2)] focus:bg-primary-600 focus:shadow-[0_8px_9px_-4px_rgba(59,113,202,0.3),0_4px_18px_0_rgba(59,113,202,0.2)] focus:outline-none focus:ring-0 active:bg-primary-700 active:shadow-[0_8px_9px_-4px_rgba(59,113,202,0.3),0_4px_18px_0_rgba(59,113,202,0.2)] driverPay"
                        data-te-ripple-init data-te-ripple-color="">
                        Accept
                    </button>
                </div>
            </div>
